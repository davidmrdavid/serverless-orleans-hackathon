...
namespace OrleansConnector
{
    ...
    public class Static
    {
        ...
        public static async Task<HttpResponseMessage> DispatchAsync(HttpRequestMessage requestMessage, Action<ISiloBuilder> configureOrleans, ILogger logger, CancellationToken hostShutdownToken)
        {
            ...
            {
                var _ = Task.Run(() => StartSiloAndDispatcher(requestMessage, configureOrleans, logger, hostShutdownToken));
            }
        ...
        public static async Task StartSiloAndDispatcher(HttpRequestMessage requestMessage, Action<ISiloBuilder> configureOrleans, ILogger logger, CancellationToken hostShutdownToken)
        {
            try
            {
